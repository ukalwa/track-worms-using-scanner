% Record the worm positions in each well in a well plate every hour to excel sheet
clc
clear all
close all
% Select folder containing all the scanned images of wells in a well plate
folder=uigetdir;
files=dir(folder);

% Create an empty excel file for recording the positions of worms
...
[image,index,name] = next_image(files,1,1);
...
imshow(image);
hold on
%divide picture into subsections because of high resolution scans
...
hold on;
text(10,20,['File:' name '.jpg'],'Color','red','FontSize',14);

% Ask user to select the worms in each subsection and record worm positions
while(1)
    ...
    switch key
        case 'e'
            ...
            [image,index,name] = next_image(files,index,1);
            image = imread([folder,'\',image]);
...
% write worm positions array to an excel file
